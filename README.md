...
For public releases, this repository already includes a GitHub Actions workflow that builds the OpenWrt package when a version tag such as `v0.9.23` is pushed. See [`docs/PUBLISHING.md`](docs/PUBLISHING.md) for the full GitHub publishing flow.
...
Version 0.9.23 is a tested pilot release with GitHub Release package publishing, SSH install/update commands, dashboard self-update support, Speedify runtime controls, safer LAN/uHTTPd binding, LAN DHCP DNS editing and one-click installation of missing lightweight modules. It is suitable for early public testing, with the compatibility limits documented above. Additional router models and OpenWrt releases should be tracked through GitHub issues before calling it broadly stable.
