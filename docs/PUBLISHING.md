...
4. Create and push a version tag matching `VERSION`, such as `v0.9.89`.
...
6. Confirm that the Release contains:
   ...
   - versioned `.apk`, such as `luci-app-ark-router-lite-0.9.89-r1.apk`
   - versioned Full `.apk`, such as `luci-app-ark-router-full-0.9.89-r1.apk`
...
git status
git add .
git commit -m "Release ARK Router v0.9.89"
git push origin main
git tag -a v0.9.89 -m "ARK Router v0.9.89"
git push origin v0.9.89
```
...
```text
https://github.com/Despensativo/ark-router/releases/tag/v0.9.89
```
