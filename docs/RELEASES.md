...
2. Create a version tag matching `VERSION`, for example `v0.9.90`.
...
- \pk\ based OpenWrt releases download \luci-app-ark-router.apk\ for Lite or \luci-app-ark-router-full.apk\ for Full.
...
Release APK assets are kept under \dist/sdk\ for the tested build, while local intermediate artifacts remain ignored. For local APK-based testing, run \scripts/build-apk-wsl.sh\; it uses the matching OpenWrt SDK \pk mkpkg\ tool to create a oarch\ LuCI package without compiling a full firmware target.
...
| Item | Value |
| --- | --- |
| Router | Cudy WR3000 v1; Acer Predator Connect W6x; D-Link DGL-5500 |
| Firmware | OpenWrt 25.12.5 r33051-f5dae5ece4; OpenWrt 24.10.x |
| Target | \mediatek/filogic\; \th79/generic\ |
| CPU/package arch | \arch64_cortex-a53\; \mips_24kc\ |
| Package manager | \pk\ / \opkg\ |
