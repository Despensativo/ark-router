...
2. Create a version tag matching `VERSION`, for example `v1.0.2`.
...
Release APK/IPK assets are kept under `dist/sdk/` for the tested build, while local intermediate artifacts remain ignored. For local package-based testing, run `scripts/build-apk-wsl.sh` or `scripts/build-ipk-wsl.sh`; they use the matching OpenWrt SDK tools to create a `noarch` LuCI package without compiling a full firmware target.
