...
echo "================================================="
echo "   ARK Router v0.9.34 - Deploy de Teste          "
...
scp -O -r -o StrictHostKeyChecking=no "$REPO_DIR/root/usr/lib/ark" "root@$ROUTER_IP:/usr/lib/"
...
scp -O -o StrictHostKeyChecking=no "$REPO_DIR/root/usr/sbin/starlink-telemetry-mailer" "root@$ROUTER_IP:/usr/sbin/starlink-telemetry-mailer"
scp -O -o StrictHostKeyChecking=no "$REPO_DIR/root/usr/sbin/starlink-telemetry-daemon" "root@$ROUTER_IP:/usr/sbin/starlink-telemetry-daemon"
