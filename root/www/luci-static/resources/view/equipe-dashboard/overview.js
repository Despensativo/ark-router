...
document.querySelector('head').appendChild(E('link', {
	...
	'href': L.resource('view/equipe-dashboard/overview.css') + '?v=' + (window.ARK_VERSION || '0.9.99')
}));
...
return view.extend({
				...
				E('div',{class:'ex-update-card'},[
					E('div',{class:'ex-feature-copy'},[
						E('div',{class:'ex-feature-name-row'},[
							...
							E('span',{class:'ex-pill online'},[update.current||window.ARK_VERSION||'0.9.99'])
						]),
	...
	enableWireguard: function(){
		...
		return fs.exec('/usr/sbin/equipe-dashboard-control', ['wireguard-enable']).then(function(r){
			...
			ui.hideModal();
			reloadSoon('WireGuard ativado com sucesso. Recarregando…', 1500);
		}).catch(function(e){
			...
			E('div', {class:'right'}, [
				...
				E('button', {class:'btn cbi-button cbi-button-negative', 'click':function(){
					return fs.exec('/usr/sbin/equipe-dashboard-control', ['wireguard-disable']).then(function(r){
						...
						ui.hideModal();
						reloadSoon('WireGuard desligado com sucesso. Recarregando…', 1200);
					}).catch(function(e){
						ui.addNotification(null, E('p', {}, [e.message]), 'danger');
					});
				}}, ['Desligar'])
										...
										E('div', {class:'right'}, [
											...
											E('button', {class:'btn cbi-button cbi-button-negative', click:function(){
												return fs.exec('/usr/sbin/equipe-dashboard-control', ['wireguard-peer-delete', p.public_key]).then(function(delRes){
													...
													ui.hideModal();
													reloadSoon('Cliente removido. Recarregando…', 1200);
												}).catch(function(err){
													ui.addNotification(null, E('p', {}, [err.message]), 'danger');
												});
											}}, ['Excluir'])
	...
	adblockCard: function(){
		...
		const toggleInput = E('input', {
			...
			change: function(ev) {
				...
				fs.exec('/usr/sbin/equipe-dashboard-control', ['adblock-toggle', desired ? '1' : '0']).then(function(r) {
					...
					reloadSoon(desired ? 'Bloqueador ativado. Recarregando…' : 'Bloqueador desativado. Recarregando…', 1200);
				}).catch(function(e) {
		...
		return E('section', { class: 'ex-card ex-adblock-card' }, [
			E('div', { class: 'ex-card-title' }, [
				...
				E('span', { class: 'ex-pill ' + pillClass }, [statusText])
			]),
			E('p', { class: 'ex-muted' }, [
	...
	renderStarlinkTelemetrySection: function(){
		...
		// Master Toggle behavior: expands/collapses details, and auto-saves enabled=0 when turned off
		telEnabledInput.addEventListener('change', L.bind(function() {
			...
			if (active) {
				...
				statusBadge.textContent = 'ATIVO / AGUARDANDO';
				statusBadge.className = 'ex-pill standby';
			} else {
	...
	showFeatureCenter: function(){
		...
		const isLegacy = !!(this.capabilities && this.capabilities.hardware && this.capabilities.hardware.is_legacy_owrt);
		const currentTheme = this.capabilities.current_theme || 'ark';
		const themeOptions = [
			E('option',{value:'ark'},['⚡ Tema ARK (Nativo)'])
		];
		if (currentTheme === 'bootstrap') {
			themeOptions.push(E('option',{value:'bootstrap'},['Tema Bootstrap (Padrão clássico)']));
		}
