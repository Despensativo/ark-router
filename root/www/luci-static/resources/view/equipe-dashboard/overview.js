...
const FEATURE_META={
	...
	argon:{name:'Tema Argon',description:'Tema visual recomendado para a melhor experiência com o ARC Router.',recommended:true},
...
return view.extend({
	...
	applyBrand: function(title){
		title=String(title||'ARC Router');document.title=title+' · OpenWrt';
	...
	update: function(data) {
		...
		const qe=sqm.wan1&&sqm.wan1.enabled==='1'; setPill('ex-qos-status',qe?'online':'standby',qe?'ATIVO':'DESLIGADO'); text('ex-qos-wan',((Number(qos.wan1_upload_kbps)||0)/1000).toFixed(1)+' Mbps total'); text('ex-qos-guest',((Number(qos.guest_upload_kbps)||0)/1000).toFixed(1)+' Mbps visitantes'); text('ex-dns',(wan['dns-server']||['1.1.1.1','8.8.8.8']).join('  •  '));
	...
	showFeatureCenter: function(){
		...
		const brandName=E('input',{class:'cbi-input-text',type:'text',maxlength:40,value:this.capabilities.title||'ARC Router','aria-label':translateText('Nome do painel')});
	...
	render: function(loaded) {
		...
		const root=E('div',{class:'ex-dashboard'},[
			...
			E('section',{class:'ex-card ex-qos-card'},[E('div',{class:'ex-card-title'},[E('div',{},[E('span',{class:'ex-kicker'},['CONTROLE DE FILAS']),E('h3',{},['CAKE / SQM'])]),E('span',{id:'ex-qos-status',class:'ex-pill standby'},['—'])]),E('div',{class:'ex-grid ex-grid-3 ex-qos-grid'},[infoRow('WAN principal','ex-qos-wan'),infoRow('Rede visitante','ex-qos-guest'),infoRow('DNS do roteador','ex-dns')]),E('a',{class:'ex-button',href:L.url('admin/system/commands')},['Abrir controles do QoS'])]),
