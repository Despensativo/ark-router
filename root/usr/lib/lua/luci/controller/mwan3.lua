...
function diagnosticsData(interface, task, custom_target)
	...
	if device then
		...
		elseif task == "ping_custom" or task == "ping_1111" or task == "ping_google" then
			local host = "1.1.1.1"
			if task == "ping_1111" then
				host = "1.1.1.1"
			elseif task == "ping_google" then
				host = "google.com"
			else
				host = custom_target or luci.http.formvalue("target") or luci.http.formvalue("custom_target") or "1.1.1.1"
			end
			host = host:gsub("%s+", "")
			if host ~= "" and host:match("^[a-zA-Z0-9%-%.:_]+$") then
				diag_command("ping -I %s -c 5 -W 1 %s 2>&1", device, host)
			else
				luci.http.write("Endereço IP ou site inválido: " .. (host or "nil"))
			end
		elseif task == "traceroute_custom" then
			local host = custom_target or luci.http.formvalue("target") or luci.http.formvalue("custom_target") or "1.1.1.1"
			host = host:gsub("%s+", "")
			if host ~= "" and host:match("^[a-zA-Z0-9%-%.:_]+$") then
				diag_command("traceroute -i %s -q 1 -w 1 -m 15 %s 2>&1", device, host)
			else
				luci.http.write("Endereço IP ou site inválido: " .. (host or "nil"))
			end
